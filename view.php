	...
	<body>
		<div id="container">
			...
			<div id="content">
				...
				<p><strong>Viewing All Persons in the Database</strong><br />
					...
					<?PHP
						...
						echo(sizeof($allPersons)." persons found. Viewing results ".($min_num+1)."-".$max_num.". Showing 25 persons per page");
						echo('<hr size="1" width="200px" align="left">');
						if ($page_num > 0)
						    echo("<a href='view.php?page_num=".($page_num-1)."'>&lt;&lt; previous ".$num_results."</a> ");
					    echo(" ... ");
						if ($page_num < count($allPersons)/25)
						    echo("<a href='view.php?page_num=".($page_num+1)."'>next ".$num_results."&gt&gt</a>");
						echo('<hr size="1" width="200px" align="left">');
						...
						foreach($allPersons as $onePerson){
							...
							if($i>=$min_num && $i<$max_num){
								...
								if (in_array("guest",$onePerson->get_type()))
								    echo('<td class="searchResults"><a href="bookingEdit.php?id='.$onePerson->get_id().'">create new booking</a></td>');
							}
							echo("</tr>");
							echo("\n");
							$i++;
